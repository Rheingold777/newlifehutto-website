...
Before a public release, refresh the actual Cloudflare deployment and account configuration, confirm authorization and recovery availability, and review `cloudflare-runtime/README.md`. The recorded baseline is deployment `52ab2c5e-058b-4353-8c85-cada43d3d099`; changes to production after that record must be reconciled. Verify the privately held prior-release `deploy.zip` through the optional `--rollback-archive PATH` build argument before release. The archive is not required for portable source builds and is not copied into them.
